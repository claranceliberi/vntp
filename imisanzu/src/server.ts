 ...
 */
const SWAGGER_TITLE = 'Imisanzu Service API';
const SWAGGER_DESCRIPTION = 'Employee management and contribution aggregation API';
...
async function bootstrap(): Promise<void> {
    ...
    if (!process.env.SWAGGER_ENABLE || process.env.SWAGGER_ENABLE === '1') {
        ...
        logger.log(`Imisanzu service running on port ${process.env.API_PORT || API_DEFAULT_PORT}`);
    } catch (error) {
        logger.error('Error starting Imisanzu service:', error);
        process.exit(1);
    }
}
